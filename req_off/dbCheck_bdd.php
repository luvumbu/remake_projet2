...
// ======================================================
// 3️⃣ TABLE style (ultra complète)
$columnsStyle = [
    // 🔑 Identifiants
...
// ======================================================
// 4️⃣ TABLE projet_params (paramètres fonctionnels par projet)
$columnsProjetParams = [
...
// ======================================================
// 👤 AJOUT D’UN UTILISATEUR (insert_safe)
$userData = [
    'nom_user'      => $dbname,
...
$resultUser = $databaseHandler->insert_safe(
    'profil_user',
    $userData,
    'email_user'
);
...
if ($resultUser['success']) {
    echo "👤 Utilisateur créé (ID " . $resultUser['id'] . ")<br>";
    $idUserCreated = $resultUser['id'];
} else {
    echo "⚠️ Utilisateur : " . $resultUser['message'] . "<br>";
}

// ======================================================
// ✅ CRÉATION DU FICHIER dbCheck.php SI TOUT EST OK
if (
    $successFK1 &&
    $successFKParent &&
    $successFKStyle &&
...
?>
PHP;
    if (file_put_contents($source_dbcheck, $content) !== false) {
        echo "✅ Base créée + relations OK<br>";
        echo "📁 Fichier <b>$source_dbcheck</b> généré";
    } else {
        echo "⚠️ Tables OK mais erreur création fichier";
    }
} else {
    echo "❌ Erreur : au moins une clé étrangère a échoué";
}
